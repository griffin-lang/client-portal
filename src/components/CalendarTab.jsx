  ...
  return (
    <div onClick={() => onSelect(appt.id)} style={{ ...card, display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 13, color: 'rgb(var(--ink-rgb) / 0.5)', marginTop: 2 }}>{appt.time} · {appt.location}</div>
      </div>
...
export default function CalendarTab({ appointments, today, calView, weekSelectedIso, onSetCalView, onSetWeekSelected, onSelectAppt }) {
  ...
  const pillInactive = { background: 'transparent', color: 'rgb(var(--ink-rgb) / 0.5)' };
  ...
  for (let i = 0; i < 42; i++) {
    ...
    monthCells.push({
      ...
      isToday,
      numColor: isToday ? '#fff' : (inMonth ? INK : 'rgb(var(--ink-rgb) / 0.25)'),
  ...
  return (
    <div style={{ padding: '8px 20px 20px' }}>
      ...
      <div style={{ display: 'flex', background: 'rgb(var(--accent-rgb) / 0.08)', borderRadius: 100, padding: 4, marginBottom: 20 }}>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: '4px 2px', marginBottom: 6 }}>
            {weekdayLabels.map((wd, i) => (
              <div key={i} style={{ textAlign: 'center', fontSize: 11, fontWeight: 600, color: 'rgb(var(--ink-rgb) / 0.35)', paddingBottom: 4 }}>{wd}</div>
            ))}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: '4px 2px', background: '#fff', borderRadius: 18, border: '1px solid rgb(var(--ink-rgb) / 0.07)', padding: '10px 6px' }}>
            {monthCells.map((cell) => (
              <div
                key={cell.key}
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 5, marginBottom: 18 }}>
            {weekCells.map((cell) => (
              ...
              >
                <span style={{ fontSize: 10.5, fontWeight: 600, color: cell.isSelected ? 'rgba(255,255,255,0.75)' : 'rgb(var(--ink-rgb) / 0.4)' }}>{cell.wd}</span>
