  ...
  return (
    <div style={{ padding: '8px 20px 20px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {notifications.map((n) => (
          <div key={n.id} style={{ background: '#fff', borderRadius: 16, border: '1px solid rgb(var(--ink-rgb) / 0.07)', padding: '14px 16px', display: 'flex', gap: 12, alignItems: 'flex-start', position: 'relative' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 13.5, color: 'rgb(var(--ink-rgb) / 0.6)', lineHeight: 1.4, marginTop: 2 }}>{n.body}</div>
              <div style={{ fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.35)', marginTop: 6 }}>{n.time}</div>
            </div>
          </div>
        ))}
      </div>
